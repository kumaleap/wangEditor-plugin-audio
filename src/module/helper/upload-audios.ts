...
import { IDomEditor, createUploader } from '@wangeditor/editor'
import insertAudio from './insert-audio'
import { IUploadConfigForAudio } from '../menu/config'

function getMenuConfig(editor: IDomEditor): IUploadConfigForAudio {
  // 获取配置，见 `./config.js`
...
function getUppy(editor: IDomEditor): Uppy {
  ...
  const successHandler = (file: UppyFile, res: any) => {
    ...
    if (customInsert) {
      // 用户自定义插入音频，此时 res 格式可能不符合预期
      customInsert(res, (src, poster) => insertAudio(editor, src, poster))
      // success 回调
      onSuccess(file, res)
      return
    }
