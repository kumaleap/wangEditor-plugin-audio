...
class InsertAudioMenu implements IModalMenu {
  readonly title = t('audioModule.insertAudio')
  ...
  getModalContentElem(editor: IDomEditor): DOMElement {
    ...
    const [srcContainerElem, inputSrcElem] = genModalInputElems(
      t('audioModule.audioSrc'),
      srcInputId,
      t('audioModule.audioSrcPlaceHolder')
    )
    const [posterContainerElem, inputPosterElem] = genModalInputElems(
      t('audioModule.audioPoster'),
      posterInputId,
      t('audioModule.audioPosterPlaceHolder')
    )
    ...
    if (this.$content == null) {
      ...
      $content.on('click', `#${buttonId}`, async e => {
        ...
        await insertAudio(editor, src, poster)
